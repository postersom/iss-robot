...
def get_interaction(id):
    userinteraction = db_session.query(Userinteraction).filter(Userinteraction.id == id).one_or_none()
    return userinteraction.dump() or ('Not found', 404)


def put_interaction(interaction):
    p = db_session.query(Userinteraction).filter(Userinteraction.id == interaction['id']).one_or_none()
    id = interaction['id']
    if p is not None:
...
def delete_interaction(id):
    ...
    if userinteraction is not None:
        ...
        db_session.query(Userinteraction).filter(Userinteraction.id == id).delete()
        db_session.commit()
...
def delete_interaction_by_slot(slot_no):
    try:
        if interaction:= get_interaction_by_slot(int(slot_no)):
...
def get_interaction_by_slot(slot_no):
    userinteraction = db_session.query(Userinteraction).filter(Userinteraction.slot_no == slot_no).one_or_none()
    return userinteraction.dump() if userinteraction else None


def add_user_interaction(slot_no, title, message, picture, html, timeout):
    db_session.add(Userinteraction(slot_no=slot_no, title=title, message=message,picture=picture, html=html,
                                   timeout=timeout, created=datetime.datetime.now()))
    db_session.commit()
...
def create_user_interaction(body):
    ...
    if not tmp_return_data['error']:
        ...
        add_user_interaction(int(slot_no),
                             request_body['title'],
                             request_body['message'],
                             request_body['picture'],
                             request_body['html'],
                             int(request_body['timeout']))
...
def user_interaction(slot_location_no):
    ...
    interaction = get_interaction_by_slot(int(slot_no))
    if not interaction:
...
def slot_location_to_slot_no(slot_location):
    slot_no = 0
    if slot_location != '':
        slot_no = int(re.search(r'\d+', slot_location).group())
    return str(slot_no)
