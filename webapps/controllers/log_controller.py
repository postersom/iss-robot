...
def put_log(log):
    p = db_session.query(Log).filter(Log.id == log['id']).one_or_none()
    id = log['id']
    if p is not None:
...
def view_log_with_filter(request_body):
    testing_model = Testing()
    tmp_return = testing_model.return_data
    tmp_filter = {
        "serial_number": None,
        "result": None,
        "start_time": None,
